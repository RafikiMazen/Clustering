...
for map_filename in maps_filenames:
    df = pd.read_csv(map_filename, low_memory=False)
    maps.append(df)
os.chdir("../")
for map_filename in maps_filenames:
    cluster_num = int(map_filename[10:-4])
    if not os.path.exists("./Simulation/distances/" + map_filename[10:-4]):
        os.makedirs("./Simulation/distances/" + map_filename[10:-4])
